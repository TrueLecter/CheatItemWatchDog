...
public class NbtFactory {
	...
	private static final BiMap<Integer, Class<?>> NBT_CLASS = HashBiMap
			.create();
	...
	private enum NbtType {
		TAG_END(0, Void.class), TAG_BYTE(1, byte.class), TAG_SHORT(2,
				short.class), TAG_INT(3, int.class), TAG_LONG(4, long.class), TAG_FLOAT(
				5, float.class), TAG_DOUBLE(6, double.class), TAG_BYTE_ARRAY(7,
				byte[].class), TAG_INT_ARRAY(11, int[].class), TAG_STRING(8,
				String.class), TAG_LIST(9, List.class), TAG_COMPOUND(10,
				Map.class);
	...
	public final class NbtCompound extends ConvertedMap {
		...
		public NbtCompound putPath(String path, Object value) {
			...
			Map<String, Object> map = getMap(
					entries.subList(0, entries.size() - 1), true);
		...
		public <T> T getPath(String path) {
			List<String> entries = getPathElements(path);
			NbtCompound map = getMap(entries.subList(0, entries.size() - 1),
					false);
		...
		private List<String> getPathElements(String path) {
			return Lists.newArrayList(Splitter.on(".").omitEmptyStrings()
					.split(path));
		}
	}
		...
		if (BASE_CLASS == null) {
			try {
				...
				String packageName = Bukkit.getServer().getClass().getPackage()
						.getName();
				Class<?> offlinePlayer = loader.loadClass(packageName
						+ ".CraftOfflinePlayer");

				// Prepare NBT
				COMPOUND_CLASS = getMethod(0, Modifier.STATIC, offlinePlayer,
						"getData").getReturnType();
				BASE_CLASS = COMPOUND_CLASS.getSuperclass();
				NBT_GET_TYPE = getMethod(0, Modifier.STATIC, BASE_CLASS,
						"getTypeId");
				NBT_CREATE_TAG = getMethod(Modifier.STATIC, 0, BASE_CLASS,
						"createTag", byte.class, String.class);

				// Prepare CraftItemStack
				CRAFT_STACK = loader.loadClass(packageName
						+ ".inventory.CraftItemStack");
				CRAFT_HANDLE = getField(null, CRAFT_STACK, "handle");
				STACK_TAG = getField(null, CRAFT_HANDLE.getType(), "tag");

				// Loading/saving
				LOAD_COMPOUND = getMethod(Modifier.STATIC, 0, BASE_CLASS, null,
						DataInput.class);
				SAVE_COMPOUND = getMethod(Modifier.STATIC, 0, BASE_CLASS, null,
						BASE_CLASS, DataOutput.class);

			} catch (ClassNotFoundException e) {
				throw new IllegalStateException(
						"Unable to find offline player.", e);
			}
		}
	}
	...
	public static NbtList createList(Iterable<? extends Object> iterable) {
		NbtList list = get().new NbtList(INSTANCE.createNbtTag(
				NbtType.TAG_LIST, "", null));
	...
	public static NbtCompound createCompound() {
		return get().new NbtCompound(INSTANCE.createNbtTag(
				NbtType.TAG_COMPOUND, "", null));
	}
	...
	public static NbtCompound createRootCompound(String name) {
		return get().new NbtCompound(INSTANCE.createNbtTag(
				NbtType.TAG_COMPOUND, name, null));
	}
		...
		try {
			...
			data = new DataInputStream(
					new BufferedInputStream(
							option == StreamOptions.GZIP_COMPRESSION ? new GZIPInputStream(
									input) : input));
	...
	public static void saveStream(NbtCompound source,
		...
		try {
			...
			data = new DataOutputStream(
					option == StreamOptions.GZIP_COMPRESSION ? new GZIPOutputStream(
							output) : output);
	...
	public static ItemStack getCraftItemStack(ItemStack stack) {
		// Any need to convert?
		if (stack == null
				|| get().CRAFT_STACK.isAssignableFrom(stack.getClass()))
			return stack;
		try {
			// Call the private constructor
			Constructor<?> caller = INSTANCE.CRAFT_STACK
					.getDeclaredConstructor(ItemStack.class);
			caller.setAccessible(true);
			return (ItemStack) caller.newInstance(stack);
		} catch (Exception e) {
			throw new IllegalStateException("Unable to convert " + stack
					+ " + to a CraftItemStack.");
		}
	}
	...
	private static void checkItemStack(ItemStack stack) {
		...
		if (!get().CRAFT_STACK.isAssignableFrom(stack.getClass()))
			throw new IllegalArgumentException(
					"Stack must be a CraftItemStack.");
	...
	private Object unwrapValue(String name, Object value) {
		...
		if (value instanceof Wrapper) {
			...
			throw new IllegalArgumentException(
					"Can only insert a WrappedCompound.");
	...
	private static void setFieldValue(Field field, Object target, Object value) {
		try {
			...
			throw new RuntimeException("Unable to set " + field + " for "
					+ target, e);
		}
	}
	...
	private static Object getFieldValue(Field field, Object target) {
		try {
			...
			throw new RuntimeException("Unable to retrieve " + field + " for "
					+ target, e);
		}
	}
	...
	private static Method getMethod(int requireMod, int bannedMod,
		...
		for (Method method : clazz.getDeclaredMethods()) {
			...
			if ((method.getModifiers() & requireMod) == requireMod
					...
					&& (methodName == null || method.getName().equals(
							methodName))
		...
		if (clazz.getSuperclass() != null)
			return getMethod(requireMod, bannedMod, clazz.getSuperclass(),
					methodName, params);
	...
	private class ConvertedMap extends AbstractMap<String, Object> implements
		...
		public Object put(String key, Object value) {
			return wrapOutgoing(original.put((String) key,
		...
		private Iterator<Entry<String, Object>> iterator() {
			final Iterator<Entry<String, Object>> proxy = original.entrySet()
					.iterator();
